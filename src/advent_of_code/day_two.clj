...
(defn get-update-fn-with-aim [line]
  (if (string? line)
    (cond
...
(defn position-updater [get-update]
  (fn [pos line]
    (let [upd (get-update line)]
      (upd pos))))
...
(defn task-a [fname]
  (->
   (reduce (position-updater get-update-fn) (starting-position) (file-lines fname))
   (multiply)))

(defn task-b [fname]
  (->
   (reduce (position-updater get-update-fn-with-aim) (starting-position) (file-lines fname))
   (multiply)))
